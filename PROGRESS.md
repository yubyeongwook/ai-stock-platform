...
- **노무체크AI(사장님 개인 사업)를 4번째 클라이언트로 등록** (`clients/laborcheck-ai.json`) — 평생교육사 자격 기반 노무 교육 프로그램(개인 영업). `labor_firm` 버티컬로 자동 분류 확인, 실제 LLM로 블로그 초안 생성해 평생교육사 자격이 자연스럽게 커리큘럼 차별점으로 녹아든 것 확인 (사실 왜곡 없음)
...
- **주식 자동화 자산(aistoag.com/aigoid.blogspot.com)을 이 시스템에 등록할지 여부** — 이미 별도 도구로 완전 자동화(블로그+릴스+카드뉴스+유튜브) 운영 중이라, 지금 파이프라인(영상 제작 미지원)에 편입할 실익이 있는지 확인 필요. 어떤 도구로 자동화했는지부터 파악 필요
...
노무체크AI(개인 노무 교육 사업)를 4번째 클라이언트로 등록. 사장님이 주식 콘텐츠(aistoag.com/aigoid.blogspot.com)가 릴스·카드뉴스·유튜브까지 이미 완전 자동화되어 있다고 밝힘 — 어떤 도구인지, 이 시스템에 편입할지는 다음 확인 필요.
